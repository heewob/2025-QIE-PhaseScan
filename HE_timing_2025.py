...
def read_map():
    with open("Lmap_ngHE_N_20200212.txt") as f:
        next(f)
        for line in f:
            l = line.split()
            geo = [int(l[0])*int(l[1]), int(l[2]), int(l[4])]        #[side*eta, phi, depth]
            ele = l[6] + l[12] + str((int(l[10])-1)*12 + int(l[11])) #RBX + RM + qiech (in 1 QIE for a RBX/RM, there are 12 QIECH (for HE))
            mapka[ele] = geo
...
tree = ElementTree()
tree.parse(args.xml_file) # most recent QIE phase setting 2024-7-18
root = tree.getroot()
...
def loop():
    ...
    tree.write('HE_QIEscanSetting_timing'+today+'.xml') # new QIE phase setting with adjustments made after 2025 
