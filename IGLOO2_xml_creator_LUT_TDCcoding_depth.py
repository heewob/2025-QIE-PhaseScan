...
HB_tp1 = [18, 12, 12, 12] #end of prompt for [depth 1, depth 2, depth 3, depth 4]
HB_tp2 = [24, 18, 18, 18] #end of slightly delayed for [depth 1, depth 2, depth 3, depth 4] 1 is 0.5ns


#ScanShift = 4 # increasing this moved TDC LUT forward (larger delayed range) by half ns steps
...
def buildPerHBtree(CFGBrickset, RBX="HB0"):
  ...
  Parameter3.text = "HB_TDC_2024_v1"
        ...
        for board in qie_board:
                for channel in qie_channel:
                        ...
                        if (irm == 5):
                            ...
                            t_p1 = HB_tp1[int(depth)-1]
                            t_p2 = HB_tp2[int(depth)-1]
                        ...
                        if (t_p1 > -1):
                            ...
                            binary_encoding = error_code*14 + delay2*(49-t_p2) + delay1*(t_p2 + 1) # if t_p2 = 1, need delay1 to continue for 2 steps (1ns)
...
def buildTree(site="904"):
  ...
  tree.write("HBTDCLUT_prompt_delayed_2024_March_%s.xml" %site, xml_declaration=True, encoding='utf-8', method="xml")
